...
**📋 Voir le fichier de commandes** : [PROD-28327_COMMANDES.md](./PROD-28327_COMMANDES.md)
...
## 📝 RÉSUMÉ SIMPLE - Session du 25 Nov 2025

### Problème Initial

**Écart comptable 4510/7000** sur l'agence FONCIA BRETAGNE (fonciaLogin: 0709) malgré l'exécution du PDATA de lettrage CAF.

### 🐛 Bug Critique Découvert et Corrigé

**Fichier** : `letter-caf-payments.ts`

**Problème** : Le filtre MongoDB ne marchait pas correctement.

```typescript
// ❌ AVANT (MAUVAIS) - trouvait 47 CafTransfers
{
  "beneficiaries.kind": "Lease",
  "beneficiaries.letteringStatus": "TO_DO"
}
// Ce filtre matchait si UN bénéficiaire était Lease ET UN AUTRE était TO_DO
// (pas forcément le même !)

// ✅ APRÈS (BON) - trouve 0 CafTransfers actuellement
{
  beneficiaries: {
    $elemMatch: {
...
// Ce filtre matche uniquement si LE MÊME bénéficiaire est Lease ET TO_DO
```

**Impact** : Le script traitait 47 faux positifs au lieu du vrai CafTransfer problématique.

### 📊 Analyse FONCIA BRETAGNE

**Écart actuel** : 15€ seulement (au lieu de 2,421€ initial)

#### Comptes Comptables

```
Compte 4510 (locataires) :  1,326,924.67€
Compte 7000 (produits)   : -1,326,909.67€
───────────────────────────────────────────
Écart                    :         15.00€
```

#### Écritures Non Lettrées (NONE) sur le Compte 7000

**Total** : 621,279 écritures non lettrées

**Répartition par ProcessCode** :
...
#### Bailleurs avec Gros Écarts

**196 bailleurs** ont des écarts > 1000€ en valeur absolue.

**Top 3** :
1. Bailleur 200354655 : -48,600.78€
2. Bailleur 200480920 : -41,200.52€
3. Bailleur 200470330 : -39,800.25€

**Mais** : Ces écarts se compensent algébriquement pour donner seulement 15€ au total.

### 🔍 Explication ORIGINE_INCONNUE

**C'est quoi ?** : C'est le `processCode` par défaut assigné automatiquement.
...
export const AccountingProcessCode = {
  // ...
  DEFAULT: "ORIGINE_INCONNUE",  // ← Valeur par défaut
  // ...
} as const;
```
...
1. **Migration de données** : Écritures importées depuis l'ancien système sans processCode
2. **Code legacy** : Écritures créées avant la mise en place du système de processCode (avant 2020-2021)
3. **Imports manuels** : CSV ou scripts sans processCode fourni

**Pourquoi c'est un problème ?**
- ❌ Pas de contexte métier (impossible de savoir quel processus a créé l'écriture)
- ❌ Pas de lettrage automatique possible
- ❌ Pas de traçabilité pour l'audit

### 🛠️ Scripts Créés/Améliorés

#### 1. `letter-caf-payments.ts` (Reusable PDATA)

**Améliorations** :
- ✅ Filtre MongoDB corrigé avec `$elemMatch`
- ✅ Pagination avec cursor Mongoose
- ✅ Utilisation des enums au lieu de strings hardcodés
- ✅ Logging par batch (tous les 100)
- ✅ Lookup d'agence correct (AgencyModel au lieu de AssociateModel)
...
- `periodMonths` : Période de rattrapage en mois (défaut: 6)

#### 2. `analyze-7000-gaps.ts` (Script d'Analyse)

**Nouveau script** transformé en `ReusableScript` pattern.
...
- `minGapAmount` : Montant minimum en € (défaut: 1000)

**Ce qu'il fait** :
1. Identifie les bailleurs avec écarts > montant minimum
2. Analyse les écritures NONE pour chaque bailleur
3. Identifie les top 5 processCode par bailleur
4. Génère un rapport détaillé avec recommandations

### ✅ Conclusion

1. **Le script CAF fonctionne maintenant correctement** grâce au fix du filtre `$elemMatch`
2. **L'écart de 15€** est normal et provient de situations historiques/exceptionnelles
3. **Les 244,915 écritures ORIGINE_INCONNUE** sont des données de migration qu'il faudra traiter à part
4. **Les 196 bailleurs avec gros écarts** se compensent entre eux (total = 15€)

### 🎯 Actions Recommandées

**Court terme** :
- ✅ Bug du filtre corrigé
- ✅ Script d'analyse créé
- ⏳ Accepter l'écart de 15€ comme normal

**Moyen terme** :
- Créer un PDATA pour analyser et catégoriser les écritures `ORIGINE_INCONNUE`
- Identifier si certaines peuvent être lettrées manuellement

**Long terme** :
- Monitorer les messages `caf-lettering` qui échouent
- Créer des écritures de régularisation pour solder les comptes historiques

---

## 📝 UPDATE - Session du 28 Nov 2025

### Nouveau Cas Découvert : CafTransfers CANCELLED

**Problème identifié** : Il faut aussi traiter dans le ML-Admin les CafTransfers avec statut **CANCELLED** dont les bénéficiaires ne sont pas lettrés.

**Explication technique** :
- Lors de l'annulation d'un CafTransfer, une écriture **4510** (compte locataire) a été créée
- Mais l'écriture **7000** (produits) correspondante n'a pas été générée
- **Résultat** : Écart comptable entre 4510 et 7000
...
### Mise à jour du Script Reusable

Le script `letter-caf-payments.ts` a été mis à jour pour inclure les CANCELLED dans le filtre :
...
**Date de résolution** : 25 Nov 2025
**Mise à jour** : 28 Nov 2025
**Écart résiduel accepté** : 15€
